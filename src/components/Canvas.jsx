...
export default function Canvas({ shapes, selectedId, selectedShape, onSelect, onDeselect, onUpdate, tool, onAddShape }) {
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    if (d.type === 'move') {
      ...
      onUpdate(d.id, { rotation: ((d.origRotation + angle - d.startAngle) % 360 + 360) % 360 })
      ...
      const local = unrotatePoint(pt.x, pt.y, cx, cy, orig.rotation || 0)
      const start = unrotatePoint(d.startX, d.startY, cx, cy, orig.rotation || 0)
      ...
      onUpdate(d.id, { x, y, width, height })
    }
  }, [onUpdate])
  ...
  const handleShapeMouseDown = useCallback((e, id) => {
    ...
    if (!shape) return
    const pt = getSVGPoint(e)
    dragRef.current = { type: 'move', id, startX: pt.x, startY: pt.y, origX: shape.x, origY: shape.y }
  ...
  const handleResizeStart = useCallback((e, handleId) => {
    ...
    const pt = getSVGPoint(e)
    dragRef.current = {
      type: 'resize', id: selectedShape.id, handle: handleId,
      startX: pt.x, startY: pt.y, orig: { ...selectedShape },
    }
  }, [selectedShape])
  ...
  const handleRotateStart = useCallback((e) => {
    ...
    dragRef.current = {
      type: 'rotate', id: selectedShape.id, cx, cy,
